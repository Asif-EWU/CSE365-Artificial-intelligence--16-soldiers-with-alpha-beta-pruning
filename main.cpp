...
void user_move();
int  minimax(bool, int, int, bool, int, int, int, int, int, int);
void opponent_move();
...
int main(int argc, char** argv)
{
    ...
    glutInitWindowPosition(100, 25);            // window will appear at this coordinate
    glutInitWindowSize(winWidth, winHeight);    // window size in pixels
    glutCreateWindow("16 Soldiers");            // text to show in the custom window bar
...
int minimax(bool is_first_iteration, int level, int turn, bool capture, int xcord, int ycord, int dead_red, int dead_blue, int alpha, int beta)
{
    if(level == minimax_depth_limit) return (dead_blue - dead_red * 2);
    if(!count_blue_soldier) return 100;
    if(!count_red_soldier) return -100;

    int res, ans, deleted_position, oldX, oldY, newX, newY, midX, midY;
    ...
    if(turn == RED)             // RED tries to maximize
    {
        ...
        for(auto i : red_soldiers)
        {
            ...
            {
                ans = minimax(false, level+1, BLUE, false, newX, newY, dead_red, dead_blue, alpha, beta);           /// minimax call
                if(ans >= res)
                {
                    res = ans;
            ...
            for(auto j : copy_suggestions)
            {
                ...
                if(j.suggest_is_captured)
                {
                    ...
                    ans = minimax(false, level, RED, true, newX, newY, dead_red, dead_blue+1, alpha, beta);        /// minimax call
                    if(ans >= res)
                    {
                        res = ans;
                        if(is_first_iteration)
    ...
    if(turn == BLUE)            // BLUE tries to minimize
    {
        ...
        for(auto i : blue_soldiers)
        {
            ...
            if(capture && !(int)copy_suggestions.size())
            {
                ans = minimax(false, level+1, RED, false, newX, newY, dead_red, dead_blue, alpha, beta);           /// minimax call
                if(ans <= res) res = ans;
            }
            ...
            for(auto j : copy_suggestions)
            {
                ...
                if(j.suggest_is_captured)
                {
                    ...
                    ans = minimax(false, level, BLUE, true, newX, newY, dead_red+1, dead_blue, alpha, beta);        /// minimax call
                    if(ans <= res) res = ans;
...
void opponent_move()
{
    ...
    minimax(true, 0, RED, is_captured, x, y, 0, 0, -INF, INF);         /// minimax call
...
void draw_lines()
{
    ...
    for(int i=0; i<(int)line_start.size(); i++)
    {
        glVertex2f(line_start[i].first, line_start[i].second);
        glVertex2f(line_end[i].first, line_end[i].second);
    }
    glEnd();
}
...
void draw_suggested_points()
{
    ...
    glColor3f(0.5, 0.0, 1.0);
    for(auto it = suggestions.begin(); it != suggestions.end(); it++)
        glVertex2f(it->suggest_x, it->suggest_y);
    glEnd();
}
...
void draw_game_over()
{
    ...
    for(int i=0; i<4; i++)
        glVertex2f(play_again[i].first, play_again[i].second);
